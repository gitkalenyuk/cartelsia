// E2E #11e: повний цикл + dialog-aware /keys: діалог "Create API key" відкривається сам — працюємо ВНУТРІШНЬО нього
import tls from "tls"
import fs from "fs"
...
p2.on("requestfinished", (req) => {
  ...
  Promise.resolve().then(() => res.text()).then(b => {
    body = b || ""
    const interesting = /sk_car_/i.test(body) || /api_keys/i.test(u) || (req.method() === "POST" && /clerk\.cartesia\.ai|api\.cartesia\.ai/.test(u))
    if (interesting) {
      const rec = { method: req.method(), status: res.status(), url: u.slice(0, 220), reqBody: (req.postData() || "").slice(0, 500), respBody: body.slice(0, 1500) }
      if (!captured.some(x => x.url === rec.url && x.method === rec.method && x.reqBody === rec.reqBody)) {
        captured.push(rec)
        console.log("[KEY-XHR] " + rec.method + " " + rec.status + " " + u.slice(0, 150))
        console.log("   req: " + rec.reqBody.slice(0, 250))
        console.log("   resp: " + rec.respBody.slice(0, 500))
      }
    }
  }).catch(() => {})
})
try {
  ...
  // --- діалог: що відкрито? ---
  const dialog = p2.locator('[data-slot="dialog-content"], [role="dialog"]').first()
  let dlgText = ""
  try { dlgText = (await dialog.innerText({ timeout: 3000 }) || "").slice(0, 400) } catch {}
  console.log("4c) dialog open text: " + (dlgText.replace(/\n/g, " | ").slice(0, 300) || "<none>"))

  async function ensureCreateForm() {
    const d = p2.locator('[data-slot="dialog-content"], [role="dialog"]').first()
    if (await d.isVisible().catch(() => false)) {
      const t = (await d.innerText().catch(() => "") || "")
      if (/api key/i.test(t) && await d.locator("input:visible").count()) return true // форма вже відкрита
      // якийсь інший діалог — закрити
      await p2.keyboard.press("Escape"); await p2.waitForTimeout(1200)
      if (await d.isVisible().catch(() => false)) { console.log("4d) dialog still open, closing again"); await p2.keyboard.press("Escape"); await p2.waitForTimeout(1200) }
      const btn = await p2.$('button:has-text("Create API key")')
      if (btn) { await btn.click({ timeout: 20000 }).catch(async () => { await p2.mouse.click((await btn.boundingBox()).x + 10, (await btn.boundingBox()).y + 10) }); await p2.waitForTimeout(2000) }
      return await d.isVisible().catch(() => false)
    }
    const btn = p2.locator('button:has-text("Create API key")').first()
    if (await btn.count()) { await btn.click({ timeout: 20000 }); await p2.waitForTimeout(2000); return true }
    return false
  }

  const formOpen = await ensureCreateForm()
  console.log("5) create form open=" + formOpen)
  if (formOpen) {
    const d = p2.locator('[data-slot="dialog-content"], [role="dialog"]').first()
    const btns = await d.locator("button:visible").all().then(bs => bs.map(b => (b.textContent() || "").trim()).catch(() => []))
    console.log("5b) dialog buttons: " + JSON.stringify(btns.filter(Boolean).slice(0, 10)))
    const inp = d.locator("input:visible").first()
    if (await inp.count()) { await inp.click(); await inp.type("Furia Key", { delay: 20 }); console.log("5c) typed name") }
    // submit: кнопка з "Create" у діалозі (footer), не header
    const submit = d.locator('button:has-text("Create")').last()
    if (await submit.count()) { await submit.click({ timeout: 15000 }); console.log("5d) clicked dialog submit") }
    else { await p2.keyboard.press("Enter"); console.log("5d) Enter fallback") }
  } else {
    const txt2 = await p2.evaluate(() => (document.body.innerText || "").slice(0, 400))
    console.log("5) NO FORM. page: " + txt2.replace(/\n/g, " | ").slice(0, 250))
    console.log("   buttons: " + JSON.stringify(await p2.$$eval("button", bs => bs.map(b => (b.textContent || "").trim()).filter(Boolean).slice(0, 15))))
    await p2.screenshot({ path: "C:/Users/J0hnD03/AppData/Local/Temp/e2e11e_nof.png" }).catch(() => {})
  }

  await p2.waitForTimeout(8000)
  const finalTxt = await p2.evaluate(() => (document.body.innerText || "").slice(0, 2000))
  console.log("6) final: " + finalTxt.replace(/\n/g, " | ").slice(0, 600))
  const skm = finalTxt.match(/sk_car_[A-Za-z0-9_-]{8,}/)
  console.log("6b) sk_car_ in text: " + (skm ? skm[0].slice(0, 24) + "..." : "no"))
  if (!skm) await p2.screenshot({ path: "C:/Users/J0hnD03/AppData/Local/Temp/e2e11e_final.png" }).catch(() => {})

  // --- fallback: прямі in-page fetch на кандидатні clerk-роути (виявити передній шлях create) ---
  const probeRes = await p2.evaluate(async () => {
    const out = []
    const cands = ["https://clerk.cartesia.ai/v1/api_keys", "https://clerk.cartesia.ai/api_keys", "https://clerk.cartesia.ai/v1/client/api_keys"]
    for (const url of cands) {
      for (const m of ["GET", "POST"]) {
        try {
          ...
          out.push({ url, m, status: r.status, body: t })
        } catch (e) { out.push({ url, m, err: String(e).slice(0, 80) }) }
      }
    }
    return out
  })
  for (const pr of probeRes) console.log("PROBE " + pr.m + " " + (pr.status ?? "ERR") + " " + pr.url + " :: " + (pr.body || pr.err || "").replace(/\s+/g, " ").slice(0, 150))

  await b2.close()
  fs.writeFileSync("C:/Users/J0hnD03/AppData/Local/Temp/furia_key_xhr.json", JSON.stringify({ email, captured, probeRes, finalText: finalTxt.slice(0, 2500), sk: skm ? skm[0] : null }, null, 2))
  console.log("SAVED furia_key_xhr.json — " + Math.round((Date.now() - t_all) / 1000) + "s")
} catch (e) {
  console.log("FATAL: " + (e && e.message).slice(0, 400))
  await p2.screenshot({ path: "C:/Users/J0hnD03/AppData/Local/Temp/e2e11e_fatal.png" }).catch(() => {})
