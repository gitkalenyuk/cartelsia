// E2E #7: як e2e6, АЛЕ чекаємо на probe-report (POST -> 204) перед забором token
import tls from "tls"
import fs from "fs"
...
// --- 0) specter token З ДОКАЗОМ, що probe-report долетів (204) ---
let specter = null
...
const reportSeen = new Promise((res) => {
  page.on("response", (r) => { if (r.request().method() === "POST" && /\/probe$/.test(r.url())) res({ status: r.status(), url: r.url() }) })
...
for (let attempt = 1; attempt <= 4 && !specter; attempt++) {
  try {
    ...
    if (!await page.evaluate(() => !!window.__clerk_specter)) throw new Error("no __clerk_specter after wait")
    console.log("0) specter in DOM, waiting for probe report (server 204)...")
    const rep = await reportSeen
    console.log("0) probe report: " + (rep ? rep.status + " " + rep.url.slice(0, 90) : "TIMEOUT (25s, continuing anyway)"))
...
console.log("0) cid=" + specter.cid, "token=" + PROOF.slice(0, 40) + "...", "exp=" + specter.rd.exp, "ttl_h=" + ((specter.rd.exp - Date.now() / 1000) / 3600).toFixed(1))
...
async function fullFlow(tag) {
  ...
  console.log(tag + " IMAP LOGIN=" + (await s.cmd("LOGIN " + JSON.stringify(IMAP.user) + " " + JSON.stringify(IMAP.pass))))
  ...
  console.log(tag + " PATCH protect_check=" + r5.r.status + " | ct=" + r5.r.headers.get("content-type"))
  console.log(tag + "   status=" + resp5.status + " missing=" + JSON.stringify(resp5.missing_fields) + " unverified=" + JSON.stringify(resp5.unverified_fields))
  console.log(tag + "   protect_check field: " + JSON.stringify(resp5.protect_check || null).slice(0, 300))
  // DOКАЗ: GET after PATCH
  const r6 = await clerk("GET", "/v1/client/sign_ups/" + suaId + "?" + QS)
  const resp6 = r6.j?.response || {}
  console.log(tag + " GET sua=" + r6.r.status + " | status=" + resp6.status + " | missing=" + JSON.stringify(resp6.missing_fields) + " | session_created=" + (resp6.session_created_at != null))
  const out = { email, suaId, ok: false, verifyStatus: r4.j?.response?.status, patchStatus: r5.r.status, patchResp: { status: resp5.status, missing: resp5.missing_fields, protect_check: resp5.protect_check || null }, getAfterPatch: { status: r6.r.status, suStatus: resp6.status, missing: resp6.missing_fields }, sec: Math.round((Date.now() - s0) / 1000), cookies: r5.r.headers.getSetCookie ? r5.r.headers.getSetCookie().map(c => c.split(";")[0].slice(0, 60)) : [] }
  if (resp6.status === "complete" || resp6.session_created_at != null) out.ok = true
  ...
  if (ct) {
    ...
    out.clientGet = { http: g.status, session_id: gj?.response?.session_id || gj?.response?.sessions?.[0]?.id, user_id: gj?.response?.user_id, status: gj?.response?.status }
    console.log(tag + " GET /v1/client=" + g.status + " session=" + (out.clientGet.session_id) + " user=" + (out.clientGet.user_id))
  }
  s.cmd("LOGOUT").catch(() => {}); s.sock.end()
  return out
}

console.log("=== FLOW #1 (новий proof token, probes reported) ===")
const f1 = await fullFlow("F1")
console.log("=== FLOW #2 (ТОЙ САМИЙ proof token — reuse test) ===")
ckMap.clear()
const f2 = await fullFlow("F2")

const res = { totalSec: Math.round((Date.now() - t_all) / 1000), specter: { cid: specter.cid, tokenHead: PROOF.slice(0, 60), exp: specter.rd.exp }, flow1: f1, flow2: f2 }
fs.writeFileSync("C:/Users/J0hnD03/AppData/Local/Temp/furia_e2e7.json", JSON.stringify(res, null, 2))
console.log("SAVED furia_e2e7.json")
console.log("E2E7 DONE in " + Math.round((Date.now() - t_all) / 1000) + "s")
console.log("SUMMARY F1: ok=" + f1.ok + " patch=" + f1.patchStatus + " suStatus=" + (f1.getAfterPatch?.suStatus))
console.log("SUMMARY F2: ok=" + f2.ok + " patch=" + f2.patchStatus + " suStatus=" + (f2.getAfterPatch?.suStatus))
