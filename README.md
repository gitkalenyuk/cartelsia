...
| ⚡ **До 25 потоків** | Персистентний пул воркерів: завершив акаунт → пауза → наступний. На весь прогін ровно N одночасних реєстрацій |
...
## 👑 Що нового у 2.0.1 — Master-клонування голосів

Один платний акаунт (Pro за $5/міс) = безлімітні голосові клони для всіх ваших безкоштовних ключів.

| | |
|---|---|
| 👑 **Master API-ключ** | Додається в Налаштуваннях; зберігається локально й ніколи не змішується з пулом free-ключів |
| 🎙️ **Клон по API** | POST /voices/clone — аудіокліп до 10 с у файлі або з мікрофона; ffmpeg сам обрізає тишу, нормалізує гучність (loudnorm I=-16), приводить до mono 44.1 kHz |
| ♻️ **Дедуплікація SHA-256** | Той самий аудіофайл ніколи не клонується двічі — повторна спроба повертає вже створений voice_id |
| 🌍 **Авто-публікація** | Одразу після клонування голос робиться публічним (PATCH access=public) і з'являється у бібліотеці — тепер він доступний ЛЮБОМУ вашому ключу, включно з безкоштовними з автореєстрації |
| ⚖️ **Rate-limit автоповтори** | 429/5xx повторюються ×5 з exponential backoff + jitter (ліміт паралельності Pro=3 налаштовується), інші 4xx — не глотаються, показуються як є |
| 🔁 **Гарантія цілісності** | Локальний реєстр `master_voices.json` + синхронізація GET /voices?is_owner=true — crash між кроками не задублює клонування |

## 🆕 Що нового у 2.1.2 — автореєстрація виправлена + Проксі 2.0

**Причина зламання реєстрації знайдена і виправлена:** Cartesia **забанила слово «cartelia»** в email/іменах (живо підтверджено: 5/5 спроб зі старим патерном → «This action couldn't be completed»).

| | |
|---|---|
| 🎭 **4 стилі email** | Random (`mt8kb1dc8llr@`), Слова (`stone42river@`), Support (`support00103…@`), Свій префікс. Бренд-слова ніколи не зʼявляються |
| 🧑 **Людські імена** | John Smith, Maria Wright… замість кривих наборів з email. Регрес-тест гарантує відсутність бренд-слів |
| 🛑 **Стоп реально вбиває** | Зупинка автореєстрації = смерть усіх браузерів миттєво. Закриття вікна = повний вихід (без фонових процесів). На старті — авто-прибирання сиріт Chromium |
| 🌐 **Проксі 2.0** | Імпорт з файлу · realtime-чекінг (статус в міру перевірки) · потоки 1–50 · таймаут · стоп чекінгу · експорт · persist · socks5 |
| 📝 **Композер** | textarea авто-розширюється під великий текст |
| 🎨 **Іконка** | Нове лого YT Cartel у вікні/сайдбарі |

Доведено живими реєстраціями: word-стиль — **повна рега за 29 с**, support — **за 35 с**.

## 🆕 Що нового у 2.1.1 — модель sonic-3.6 + locale

Cartesia випустила **sonic-3.6** (GA) — Cartelsia оновлена одразу:

| | |
|---|---|
| 🚀 **sonic-3.6 за замовчуванням** | Вибір у композері й Налаштуваннях (3.6 / 3.5 / 3). Стара 3.5 і 3 досі працюють — API не зламано (перевірено наживо) |
| 🌍 **Locale** | Нове поле API: регіональні правила читання дат/чисел (en-GB, uk-UA, de-DE…). Видиме для 3.6; взаємовиключне з мовою — застосунок сам шле лише одне з двох |
| ✅ **Жива верифікація** | bytes + SSE + locale + emotion на 3.6 — усе 200; тести 139/139 |

## 🌐 Що нового у 2.1.0 — Спільні голоси + кеш TTS

Будь-який публічний голос Cartesia (свій клон із Share або чужий shared-голос) тепер можна додати
за Voice ID і озвучувати ним **усім пулом безкоштовних ключів**.

| | |
|---|---|
| 🌐 **Реєстр спільних голосів** | Клон голосу → «Спільні голоси»: вставляєте Voice ID (кнопка Share → Shared на play.cartesia.ai дає ID) → аліас → голос у бібліотеці. Нічого не копіюється в Cartesia — зберігається локальний ярлик alias → voice_id (копіювати чужі голоси API не вміє і не буде) |
| 🎯 **Як це працює** | Публічний голос можна ВИКОРИСТОВУВАТИ будь-яким ключем просто за ID: кредити списуються з ВАШОГО ключа, а не з акаунта власника (перевірено наживо). Отже 100 000 символів тексту розподіляться по всім free-ключам пулу (по ~20 000 кожен), але озвучаться ОДНИМ голосом |
| 🛡️ **Захист від ревокації** | Власник може вимкнути Share будь-якої миті. Перед КОЖНИМ прогоном Cartelsia сама перевіряє голос (GET /voices/{id}); якщо він став приватним — прогін ЧЕСНО зупиняється з чітким повідомленням, а не мовчки озвучує іншим голосом |
| 💾 **Кеш TTS** | sha256(voice+model+language+text) → `data/tts-cache/`. Переозвучка того самого тексту = 0 кредитів, миттєво. Crash-safe атомарний запис (tmp → rename) |
| ✅ **Перевірено живим API** | GET /voices/{id} чужим ключем → is_owner:false; TTS чужим free-ключем → справжній WAV; семантика Shared = PATCH access=public підтверджена офіційною документацією |

### 📮 Інструкція: свій клон → Shared → озвучка будь-яким free-ключем

1. Створіть клон на сайті: [play.cartesia.ai](https://play.cartesia.ai) → Voices → **Clone voice** (аудіо до 10 с).
2. У картці клона натисніть **Share** → оберіть **Shared** («Anyone with the voice ID can use it»).
3. Скопіюйте **ID** з того ж діалогу (рядок `afe1bd4e-954e-…`).
4. У Cartelsia → **Клон голосу → Спільні голоси** → вставте ID → придумайте аліас (наприклад `don-pedro`) → **Додати**.
5. Готово: голос назавжди у вашій бібліотеці, вибирається в композері й озвучується **будь-яким ключем пулу** — включно з усіма free-ключами, нареганими автореєстрацією 2.0.
...
| 👑 **Master-клонування (2.0.1)** | Pro-акаунт як «фабрика голосів»: клон по API з будь-якого аудіокліпу ≤10 с, авто-публікація, дедуплікація — і клон озвучується всім пулом безкоштовних ключів |
| 🌐 **Спільні голоси + кеш (2.1.0)** | Додавайте будь-який публічний голос за Voice ID — озвучуйте ним усім пулом free-ключів (кредити з вашого ключа, не власника). Пребатч-перевірка ревокації. Кеш TTS: повтори безкоштовні |
